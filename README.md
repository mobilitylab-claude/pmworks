...
- **버전 관리 수칙**:
    - **Current Version**: `v2.1.0` (UI 및 `package.json` 동기화됨)
    - **Major**: 아키텍처 변경 또는 DB 스키마 v3+ 전환 시
    - **Minor**: 신규 기능 추가 또는 대규모 UI 개편 시
    - **Patch**: 버그 수정 및 미세 조정 시
    - 모든 기능 수정 후 Push 시점에 버전을 갱신하며, 이 규칙은 프로젝트의 최우선 관리 수칙으로 유지됩니다.
